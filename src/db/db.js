...
const root = __dirname.replace(/\\/g, "/").split("/src")[0].concat("/src");
...
fs.readdir(root, (err, files) => {
  ...
  files.forEach((file) => {
    ...
    if (isDir) {
			fs.readdir(path.join(root, file), (err2, childFiles) => {
				childFiles.forEach((childFile) => {
					if (childFile.includes(".model.js")) {
						console.log(`../${file}/${childFile.split('.js')[0]}`);
						models[file] = require(`../${file}/${childFile.split('.js')[0]}`)(sequelize, DataTypes);
          }
        });
      });
    }
